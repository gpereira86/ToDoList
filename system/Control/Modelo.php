 ...
 * em um banco de dados. Ela utiliza uma conexão com o banco de dados através da classe Conexao e pode gerenciar mensagens
...
abstract class Modelo
{
     ...
     */
    private function apagar(string $termos)
    {
        try {
     ...
     * @return bool
     */
    public function atualizarOrdens(int $id, int $ordem): bool
    {

        try {
            $query = "UPDATE " . $this->tabela . " SET ordenar = " . $ordem . " WHERE id = " . $id;
            ...
            $this->erro = $ex->getMessage(); // Armazena a mensagem de erro
            error_log($this->erro); // Registra o erro no log do servidor
     ...
     * @return int
     */
    public function ultimoOrdem(): int
    {
        return Conexao::getInstancia()->query("SELECT MAX(ordenar) as maximo FROM {$this->tabela}")->fetch()->maximo;
    }
